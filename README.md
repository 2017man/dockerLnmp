...
[**[GitHub地址]**](https://github.com/2017man/dockerLnmp.git) 
...
2. `clone`项目：
    ```
    $ git clone https://github.com/2017man/dockerLnmp.git
    ```
...
4. 拷贝并命名配置文件（Windows系统请用`copy`命令），启动：
    ...
    $ cp docker-compose.sample.yml docker-compose.yml   # 复制 docker-compose 配置文件。默认启动3个服务：
                                                        ...
                                                        # 除服务块前的注释
    $ docker-compose up                                 # 启动（如若无法启动,请使用当前命令docker-compose up --build）
    ```
